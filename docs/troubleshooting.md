...
- **Invalid function parameters near `ago()` or `now()`:** the saved-search parser requires literal parameter defaults. Current `v0.1.4` assets use nullable datetime literals and apply the rolling 14-day defaults inside the KQL body.
- **`Query Update/Create is restricted to user assigned storage`:** the workspace requires customer-managed storage for saved queries, but no usable linked storage account is affiliated with data source type `Query`. Configure the Microsoft-managed workspace prerequisite before retrying; do not grant broad roles or let this foundation create customer storage implicitly. Starting August 31, 2026, creating or updating the link also requires a workspace managed identity with `Storage Table Data Contributor` on the storage account.
